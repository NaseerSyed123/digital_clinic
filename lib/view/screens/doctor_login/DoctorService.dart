...
class UserService {
  static Future<bool> browse(String query) async {
    ...
    if (query != null && query.isNotEmpty) {
      _url = 'https://learnunstoppable.com/public/api/login-user?${query}';
    ...
    if (response.statusCode != 200) {
      ...
      try {
        ...
        if (response.statusCode != 200) {
          ...
          print("Over seer User ID ${Overseer.userId} Sign In User ID ${_userList[0].data.id}");
          if (content.contains("\"FitnessIngrediants\":null")) {
            return true;
          } else {
            List<LoginModel> _userList =
                collection.map((json) => LoginModel.fromJson(json)).toList();
